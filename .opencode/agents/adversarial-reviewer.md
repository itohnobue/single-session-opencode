...
5. **Verify scope tags** — check each finding's Scope against the brief's out-of-scope line. A real bug outside the brief's scope is CONFIRMED but tagged out-of-scope: it never silently inflates task failure, and it is never dropped merely for being out-of-scope.
