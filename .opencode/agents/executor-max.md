---
description: "Executor agent (MAX reasoning) — runs AFTER the prepare agent. Use for deep-analysis and investigation executor tasks — where deeper reasoning pays off. Synthesis and implementation are mechanical: they stay on executor-high. Given a task file whose structure is: coordination template → RESEARCH DATA section (prepared domain knowledge, injected by the main model) → the task itself. Read the file, use the research data as your briefing, execute the task, write the report. No web research of its own."
mode: subagent
reasoningEffort: max
tools:
  read: true
