...
4. **Tag confidence signals:**
   - When the originating stage used a second opinion (T3): tag each finding "both-found" (both agents reported independently) or "single-found" (one agent only). Both-found signals cross-agent agreement and carries elevated confidence. Surface all tags in synthesis.
