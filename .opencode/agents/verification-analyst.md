...
8. **Write the extraction report** with a batch assignment table using EXACTLY these columns: `| Batch | Agent | Severity scope | Finding IDs | RE-EXAMINE IDs | Source report paths |`. Before writing the table, verify every batch is within the hard caps and state `max batch size: N` on the line under it. Every MEDIUM+ finding appears in exactly one batch (a finding with no batch assignment is a defect); a finding routed straight to synthesis is marked `direct-synthesis`. The table MUST already satisfy the caps — the main model spawns exactly what it lists and does not re-group.
